...
const SellerSideBar = ({ open }: { open: boolean }) => {
    ...
    return (
        <div className={` h-screen w-[20%] md:w-[15%] flex flex-col fixed top-0 left-0 z-20 bg-white shadow-sm border-r border-gray-400 p-4 overflow-y-scroll  transition-transform duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}>
            ...
            <div className="px-3 py-2.5 my-5 flex flex-col md:flex-row items-center justify-center gap-3.5 rounded-xl shadow-sm ">
                <div className="h-8 w-8 rounded-full border border-primary bg-gray-400">

                </div>
                <div className="flex flex-col md:flex-row">

                    <p className="text-sm font-medium text-center ">Alex Sandro</p>
                    <p className="font-semibold text-sm text-center">Seller</p>
                </div>
            </div>
            ...
            <div className="w-full flex items-center justify-center  absolute px-3 bottom-10 left-0">
                <button onClick={handleLogOut} className=" w-full flex flex-col md:flex-row items-center px-6 gap-2 py-2.5 hover:bg-primary-hover/5 hover:text-primary">
